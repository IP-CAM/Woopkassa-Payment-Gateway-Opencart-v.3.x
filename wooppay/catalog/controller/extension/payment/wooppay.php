...
class ControllerExtensionPaymentWooppay extends Controller
{
	...
	{
		$client = new WooppaySoapClient($this->config->get('payment_wooppay_url'));
		$login_request = new CoreLoginRequest();
		$login_request->username = $this->config->get('payment_wooppay_merchant');
		$login_request->password = $this->config->get('payment_wooppay_password');
	...
	public function invoice()
	{
		...
		try {
			if ($client = $this->login()) {
				$prefix = trim($this->config->get('payment_wooppay_prefix'));
				...
				$invoice_request->serviceName = $this->config->get('payment_wooppay_service');
